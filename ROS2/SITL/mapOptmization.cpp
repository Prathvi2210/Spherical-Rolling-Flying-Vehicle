...
public:
    ...
    void addGPSFactor()
    {
        ...
        if (cloudKeyPoses3D->points.empty())
            return;
        // EDIT 1: opened distance gate 5.0 -> 0.5 for small indoor room
        else
        {
        ...
        while (!gpsQueue.empty())
        {
            ...
            else
            {
                ...
                // EDIT 2: barometer z-only source. Take z from GPS; take x/y from
                // current SLAM estimate so the horizontal part of the factor is a
                // no-op and only z is actually corrected. Do NOT trust GPS x/y.
                ...
                // EDIT 3: zero-(x,y) init guard REMOVED -- our bridge intentionally
                // sends x=y=0, and we now override x/y from SLAM above anyway.
                // Add GPS every a few meters
                PointType curGPSPoint;
                curGPSPoint.x = gps_x;
                curGPSPoint.y = gps_y;
                curGPSPoint.z = gps_z;
                // EDIT 4: opened spacing gate 5.0 -> 0.5
                if (pointDistance(curGPSPoint, lastGPSPoint) < 0.5)
                    continue;
                else
                    lastGPSPoint = curGPSPoint;
                // EDIT 5: weak x/y (no-op, std 1e3), firm z so baro actually pulls.
                gtsam::Vector Vector3(3);
                Vector3 << 1e6, 1e6, max(noise_z, 1.0f);
